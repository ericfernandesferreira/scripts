...
rm -rf /lib/modules/4.3.1/
rm -rf /usr/src/linux-4.3.1/
rm -rf /usr/src/linux
rm /boot/config
rm /boot/vmlinuz
rm /boot/System.map
...
cp linux-4.3.1.tar.xz /usr/src
cd /usr/src
tar xvf linux-4.3.1.tar.xz
ln -s linux-4.3.1 linux
rm linux-4.3.1.tar.xz
...
cp System.map /boot/System.map-4.3.1
ln -s /boot/System.map-4.3.1 /boot/System.map
cp .config /boot/config-4.3.1
ln -s /boot/config-4.3.1 /boot/config
cd arch/x86_64/boot
cp bzImage /boot/vmlinuz-4.3.1
ln -s /boot/vmlinuz-4.3.1 /boot/vmlinuz
cp -a /etc/rc.d/rc.modules-4.1.13 /etc/rc.d/rc.modules-4.3.1

# Gerando o initramfs
# mkinitrd -c -k 4.3.1 -m ext4 -f ext4 -r /dev/sda5
