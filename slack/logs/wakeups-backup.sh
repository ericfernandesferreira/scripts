#!/bin/bash
# Monitor Unificado v30 - Ajustado para IRQs compartilhadas
...
get_friendly_name() {
    local raw_info=""
    # Agora verificamos se o i915 ou outros estão na lista de dispositivos
    case "$1" in
...
while true; do
    ...
    # AJUSTE AQUI: Captura todos os nomes da linha para evitar falsos positivos
    HW_LIST=$(paste /tmp/intr_prev /tmp/intr_curr | awk '
        NR > 1 && $1 ~ /[0-9]:/ { 
            s_p=0; s_c=0; for(i=2; i<=9; i++) { s_p+=$i; s_c+=$(i+NF/2); }
            diff = s_c - s_p;
            if (diff > 0) {
                devices = ""; for(i=11; i<=NF; i++) devices = devices $i ",";
                sub(/,$/, "", devices);
                print "HW_" devices, diff 
            }
        }')
